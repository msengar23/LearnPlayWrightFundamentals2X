...
Fast Chrome DevTools extension by The Testing Academy. Generates locators for Playwright, Selenium, Cypress, WebdriverIO, and Robot Framework — straight from any element on a page.

**No ads. No tracking. No BS. Just locators.**

Live at **snaplocator.com**.

## What it does

Adds a "SnapLocator" tab to Chrome DevTools (next to Elements, Console, Network) AND a sidebar pane inside the Elements panel (next to Styles, Computed, Layout, Event Listeners). Select any element, get every locator strategy ranked by stability:
...
npm run package     # zips dist/ → ../public/downloads/snaplocator-v{VERSION}.zip
```
...
6. Open any web page → F12 → click **SnapLocator** tab (or use the SnapLocator sidebar inside Elements panel)
...
Free for everyone. Built by The Testing Academy.
