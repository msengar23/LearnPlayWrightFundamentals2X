...
const STEPS = [
  {
    ...
    body: (
      <>
        Click the button below — saves <code className="text-sm bg-slate-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded">snaplocator-v{VERSION}.zip</code> (~73 KB).
      </>
    ),
  },
  { title: 'Unzip', body: 'Right-click the downloaded file → Extract / Unzip. Place the folder somewhere stable (e.g. ~/Tools/snaplocator/).' },
  {
  ...
  },
  {
    ...
    body: (
      <>
        On any page press <kbd className="px-2 py-0.5 border rounded text-xs">F12</kbd> (or <kbd className="px-2 py-0.5 border rounded text-xs">Cmd</kbd>+<kbd className="px-2 py-0.5 border rounded text-xs">Opt</kbd>+<kbd className="px-2 py-0.5 border rounded text-xs">I</kbd>). Look for the <strong>SnapLocator</strong> tab — or open the Elements panel and find the SnapLocator sidebar.
      </>
    ),
  },
];
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-6 py-16">
        ...
        <h1 className="text-4xl font-bold tracking-tight mb-3">Install SnapLocator</h1>
        ...
        <div className="mt-12 p-5 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900">
          ...
          <p className="text-sm text-emerald-900 dark:text-emerald-200 mt-1">
            Pin SnapLocator's sidebar inside the Elements panel — locators auto-update every time you click an element in the DOM tree. No tab switching needed.
          </p>
        </div>
      </div>
      <Footer />
