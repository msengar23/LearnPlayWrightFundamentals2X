# How SnapLocator was built — the prompt log

This document records the actual conversation that built SnapLocator from a one-liner to a v0.8.0 9-framework Chrome DevTools extension + marketing site. Every prompt below was typed by the project owner ([@PramodDutta](https://github.com/PramodDutta)). The model (Claude) implemented each prompt as a sequence of tool calls (`Read`, `Edit`, `Write`, `Bash`, sub-agent spawns).
...
— Built with Claude Code, by [The Testing Academy](https://thetestingacademy.com).
